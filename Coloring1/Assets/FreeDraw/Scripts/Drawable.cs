...
// BRUSH TYPES. Implement your own here
        ...
        {
            if ( detectClicks.isPainting && detectClicks.showImage)
            {
        ...
        void Update()
        {
            if (!detectClicks.letColoring||detectClicks.showImage)
                return;
            // Is the user holding down the left mouse button?
            bool mouse_held_down = Input.GetMouseButton(0);
            if (mouse_held_down && !no_drawing_on_current_drag)
            {
                //PointerEventData a = new PointerEventData(Input);
                //print(a.position);
        ...
        public void MarkPixelsToColour(Vector2 center_pixel, int pen_thickness, Color color_of_pen)
        {
            ...
            for (int x = center_x - pen_thickness; x <= center_x + pen_thickness; x++)
            {
                ...
                for (int y = (int)Mathf.Sqrt(pen_thickness * pen_thickness - Mathf.Pow(x - center_x, 2)) + center_y; y >= center_y-(int)Mathf.Sqrt(pen_thickness * pen_thickness - Mathf.Pow(x - center_x, 2))  ; --y)
        ...
        public void ColourPixels(Vector2 center_pixel, int pen_thickness, Color color_of_pen)
        {
            ...
            //for (int x = center_x + pen_thickness; x >= center_x + pen_thickness; --x)
            //{
            //    for (int y = (int)Mathf.Sqrt(pen_thickness * pen_thickness - Mathf.Pow(x - center_x, 2)) + center_y; y >= center_y; --y)
            //    {
            //        //малюємо нижню частину круга
            //        //drawable_texture.SetPixel(x, y, color_of_pen);
            //        //та верхню
            //        //drawable_texture.SetPixel(x, drawable_texture.height - y, color_of_pen);
            //    }
            //}
        ...
        void Start()
        {
            ...
            current_brush = PenBrush;
            GetComponent<SpriteRenderer>().sprite.texture.SetPixels(detectClicks.img.texture.GetPixels());
